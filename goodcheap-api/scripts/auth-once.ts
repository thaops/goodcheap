...
async function main() {
  ...
  const outPath = resolve(parseArg('--out') || 'storageState.json');
  const userDataDir = resolve(parseArg('--profileDir') || '.playwright-tiktok');
  ...
  writeFileSync(outPath, JSON.stringify(state, null, 2), { encoding: 'utf-8' });
